...
\begin{enumerate}
	...
	\tikzarrow{oac5}{oac1}
\end{enumerate}
\hlr{Problem:} single batch $\Rightarrow$ need parallel actor-critic (synchronous / asynchronous) \cite{mnih2016icml}
...
\begin{enumerate}
	\item \tikzmark{opac1}Take action $\textbf{a} \sim \pi_{\theta}(\textbf{a|s})$, get $(\textbf{s, a, s'}, r)$, store in {\color{red} buffer $\mathcal{B}$}
	...
	\item \tikzmark{opac5}$\theta \leftarrow \theta + \alpha \nabla_\theta J(\theta)$
	\tikzarrow{opac5}{opac1}
\end{enumerate}
...
\section{References}
\begin{itemize}
	\item \citeaustitle{sutton1999policy}
	\item \citeaustitle{mnih2016icml}
	\item \citeaustitle{schulman2015high}
	\item \citeaustitle{gu2016q}
\end{itemize}