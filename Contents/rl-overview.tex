...
\chapter{Overview}
\ac{RL} are approaches for learning decision making from experience. In the \ac{AI} context, many \ac{RL} algorithms handle the scarcity of available (human-annotated) data.
...
\begin{figure}[hbt!]
	\centering
	\begin{tikzpicture}
		\fill[green, very thick] (.35,1.25) rectangle (4.2,2.7);
		\fill[blue, very thick] (.35,-1.4) rectangle (4.2,-2.5);
		\fill[orange, very thick] (-5.7,-.7) rectangle (-1.4,.7);
		\node[black, text width=4cm] at (2.5,2) {Fit a model / estimate the return};
		\node[white, text width=4cm] at (2.5,-2) {Improve the policy};
		\node[white, text width=3.7cm] at (-3.5,0) {Generate samples (\ie run the policy)};
		\node[red, text width=3.7cm] at (-6.5,0) {Real time or simulation};
		\draw[thick, -latex] (2,1.25) -- (2,-1.4);
		\draw[thick, -latex] (.35,-2) -- (-3.5,-2) -- (-3.5,-.7);
		\draw[thick, -latex] (-3.5,.7) -- (-3.5,2) -- (.35,2);
	\end{tikzpicture}
